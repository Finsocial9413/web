...
import { fetchDetailedChatHistory } from '../services/chatService' // Add this import

// Import new component-based structure
import Greeting from "./mainLayoutComponents/Greeting"
import Sidebar from "./mainLayoutComponents/Sidebar"
...
interface MainLayoutProps {
  ...
  setCurrentChat: (chat: DetailedChatResponse | null) => void;
}
...
interface DetailedChatResponse {
  ...
  projectId?: string; // Add this optional field
}

interface ChatSelectParams {
  id: string;
  title: string;
}
...
const MainLayout: React.FC<MainLayoutProps> = ({ 
  ...
  toggleDarkMode, 
  currentChat,
  setCurrentChat  // Add this line
}) => {
  // Remove or comment out this console.log
  // console.log("MainLayout rendering with darkMode:", isDarkMode);
  ...
  // Add useEffect to handle currentChat changes
  useEffect(() => {
  ...
  const handleChatSelectWrapper = async (chat: ChatSelectParams) => {
    try {
      if (!userProfile?.name) {
      ...
      // First fetch the detailed chat history
      const chatDetails = await fetchDetailedChatHistory(userProfile.name, chat.id);
      ...
      const formattedMessages = chatDetails.messages.map((msg) => ({
        id: crypto.randomUUID(),
        content: msg.prompt + '\n\nReasoning:\n' + msg.reasoning + '\n\nResponse:\n' + msg.response,
        type: 'assistant' as const,
        timestamp: new Date(msg.timestamp),
